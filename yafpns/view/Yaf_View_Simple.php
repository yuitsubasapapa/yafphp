...
class View_Simple implements View_Interface
{
	...
	public function render($tpl_file, $tpl_vars = null)
	{
		...
		if ($this->_tmp_path = realpath($tpl_file)) {
			if ($this->_loader_import() == false) {
				ob_end_clean();
				$this->_trigger_error('Failed opening template ' . $this->_tmp_path . ':' . YAF_ERR_NOTFOUND_VIEW);
				return false;
			}
		} else {
			if (!is_string($this->_tpl_dir)) {
				ob_end_clean();
				$this->_trigger_error('Could not determine the view script path, you should call Yaf\View_Simple::setScriptPath to specific it');
				return false;
			} else {
	 ...
	 *
	 * @param string $name
	 * @return boolean | Yaf\View_Simple
	 */
	...
	private function _trigger_error($message, $code = YAF_ERR_NOTFOUND_VIEW)
	{
		if (YAF_G('throw_exception')) {
			...
			Application::app()->setLastError($message, $code);
