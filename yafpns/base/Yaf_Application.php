...
final class Application
{
	...
	public function __construct($config, $section = null)
	{
		...
		// yaf_config_instance
		if (is_string($config)) {
			$this->config = new \Yaf\Config_Ini($config, $section);
		}
		if (is_array($config)) {
			$this->config = new Config_Simple($config, true);
		}

		if (is_null($this->config)
				|| !is_object($this->config)
				|| !($this->config instanceof Config_Abstract)
		...
		$request = new Request_Http(null, YAF_G('base_uri'));
		YAF_G('base_uri', null);
		...
		if (is_null($this->dispatcher)
				...
				|| !($this->dispatcher instanceof Dispatcher)) {
			unset($this);
		...
		if (YAF_G('local_library')) {
			$loader = Loader::getInstance(YAF_G('local_library'), YAF_G('global_library'));
		} else {
			$local_library = YAF_G('directory') . '/library';
			$loader = Loader::getInstance($local_library, YAF_G('global_library'));
		}
	 ...
	 *
	 * @param void
	 * @return boolean | Yaf\Application
	 */
	public function bootstrap()
	{
		...
		if (!class_exists('Bootstrap', false)) {
			...
			if (!Loader::import($bootstrap_path)) {
				...
				if (!($bootstrap instanceof Bootstrap_Abstract)) {
					trigger_error('Expect a Yaf\Bootstrap_Abstract instance, Bootstrap give', E_USER_WARNING);
					return false;
				}
			}
		}
	...
	private function _parse_option($config = null)
	{
		...
		if (!($app instanceof Config_Abstract)) {
			$this->_trigger_error('Expected an array of application configure', YAF_ERR_TYPE_ERROR);
			return false;
		}
		...
		if (isset($app->library)) {
			...
			} elseif ($app->library instanceof Config_Abstract) {
		...
		if (isset($app->view) && ($app->view instanceof Config_Abstract)) {
		...
		if (isset($app->dispatcher) && ($app->dispatcher instanceof Config_Abstract)) {
			...
			if (isset($app->dispatcher->defaultRoute)
					&& ($app->dispatcher->defaultRoute instanceof Config_Abstract)) {
		...
		if (isset($app->system) && ($app->system instanceof Config_Abstract)) {
